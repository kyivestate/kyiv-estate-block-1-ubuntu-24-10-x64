...
project="${1:-/opt/kyiv-estate-block-2}"
user="${2:-kyivestate}"
apt-get update
...
install -m 0640 -o root -g "$user" "$project/deploy/ubuntu-24.10-x64/kyiv-estate.env.example" /etc/kyiv-estate/block-2.env
install -m 0644 "$project/deploy/ubuntu-24.10-x64/systemd/kyiv-estate-block2.service" /etc/systemd/system/kyiv-estate-block2.service
...
systemctl enable kyiv-estate-block2.service kyiv-estate-guard.service kyiv-estate-media.service
